(() => {
  ...
  function bindElements() {
    [
      ...
      "vacationFields", "vacationStartInput", "vacationEndInput",
  ...
  async function loadPublicUsers() {
    ...
    try {
      const { data, error } = await supabaseClient.rpc("upo4_list_nips");
      if (error) throw error;
      if (data?.length) populateNipSelects(data.map(row => row.nip));
    } catch (err) {
      console.error(err);
      setLoginMessage("No se pudieron cargar los NIP. Revisa la seguridad V2.6 en Supabase.", true);
    }
  }
  ...
  async function login() {
    ...
    try {
      ...
      const { data, error } = await supabaseClient.rpc("upo4_login", {
        p_nip: nip,
        p_password: password
      });
      if (error) throw error;
  ...
  function fillFormFromSelected() {
    ...
    els.vacationStartInput.value = selectedDate || "";
    els.vacationEndInput.value = selectedDate || "";
  ...
  function updateConditionalFields() {
    ...
    if (type === "V") {
      els.vacationStartInput.value = selectedDate || "";
      if (!els.vacationEndInput.value) els.vacationEndInput.value = selectedDate || "";
    }
  }
  ...
  async function saveAnnotation() {
    ...
    try {
      ...
      if (tipo === "V") {
        const startDate = els.vacationStartInput.value || selectedDate;
        const endDate = els.vacationEndInput.value || startDate;
        if (!startDate || !endDate) return setModalMessage("Indica fecha inicial y final de vacaciones.", true);
        if (parseDate(endDate) < parseDate(startDate)) return setModalMessage("La fecha final no puede ser anterior a la inicial.", true);

        const vacationDates = datesBetween(startDate, endDate).filter(dateStr => !isFreeDay(dateStr));
        if (!vacationDates.length) return setModalMessage("El rango seleccionado no contiene días de trabajo.", true);

        payload = vacationDates.map(dateStr => ({
          fecha: dateStr,
        ...
        payload = [{
          ...
          creado_por: selectedAnnotation?.creado_por || session.nip,
          actualizado_por: session.nip,
          updated_at: new Date().toISOString()
        }];
      }

      const { error } = await supabaseClient.rpc("upo4_save_annotations", {
        p_token: session.token,
        p_rows: payload
      });
      if (error) throw error;
  ...
  async function createUser() {
    ...
    try {
      const { error } = await supabaseClient.rpc("upo4_create_user", {
        p_token: session.token,
        p_nip: newNip,
  ...
  async function changeGeneralPassword() {
    ...
    try {
      const { error } = await supabaseClient.rpc("upo4_change_general_password", {
        p_token: session.token,
        p_password: password
      });
      if (error) throw error;
  ...
  async function deleteMonth() {
    ...
    const lastDay = toDateString(currentYear, currentMonth, new Date(Date.UTC(currentYear, currentMonth, 0)).getUTCDate());
    try {
      const { error } = await supabaseClient.rpc("upo4_delete_month", {
        p_token: session.token,
        p_from: firstDay,
        p_to: lastDay
      });
  ...
  function datesBetween(startDateStr, endDateStr) {
    const dates = [];
    const cursor = parseDate(startDateStr);
    const end = parseDate(endDateStr);
    while (cursor <= end) {
      dates.push(toDateString(cursor.getUTCFullYear(), cursor.getUTCMonth() + 1, cursor.getUTCDate()));
  ...
  function formatAnnotationHtml(a) {
    return `<div class="annotation-line">${escapeHtml(formatCalendarAnnotationText(a))}</div>`;
  }
